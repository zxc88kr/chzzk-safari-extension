...
크롬용 확장은 많지만 Safari에서는 쓸 수 없습니다. 그래서 자주 쓰던 기능만 골라 Safari Web Extension으로 다시 만들었습니다. 모든 기능은 확장 팝업에서 하나씩 켜고 끌 수 있습니다.
...
- **PIP · 화면 캡처** — 화면 속 화면, 현재 장면 이미지 저장
...
1. Xcode의 **Signing & Capabilities**에서 본인 Apple ID(Personal Team)를 선택
...
치지직은 빌드마다 바뀌는 css-module 해시 클래스를 쓰기 때문에, 클래스명을 직접 박지 않고 **텍스트·구조 기반 태깅**과 `data-czse-*` 속성으로 DOM을 다룹니다. 사이드바 갱신은 팔로잉 라이브 API를 직접 폴링합니다.
