...
**준비물** — macOS 14.5 이상, Xcode 16 이상 (Xcode 준비는 아래 접기 참고)
...
내려받기 → 빌드 → 설치가 자동으로 진행되어 앱이 `~/Applications` 에 설치됩니다. 처음은 **수 분** 걸리고 `▶ 빌드 중…` 에서 멈춘 듯 보여도 정상이니 터미널을 닫지 마세요.
...
<details>
<summary>Xcode 준비 (처음 한 번만)</summary>
